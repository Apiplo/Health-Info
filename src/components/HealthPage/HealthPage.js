...
// Right-side column for advertisement
...
const Home = () => {
  ...
  const healthRef = useTranslation();
  ...
  useEffect(() => {
    const loadHealthArticles = async () => {
      try {
        setLoading(true);
        setError('');
        // Fetch health-tagged articles with full payload like Home
        const url = `http://localhost:3000/api/articles/${currentLanguage}?tag=health`;
        ...
        if (Array.isArray(data)) {
          ...
          articles = data.health || data['Health'] || Object.values(data)[0] || [];
        }
        ...
        if (normalized.length) {
          console.debug('HealthPage sample article:', normalized[0]);
        }
        setHealthArticles(normalized);
      } catch (err) {
        console.error('Error loading health articles:', err);
      } finally {
        setLoading(false);
      }
    };
      ...
      <HomeContainer ref={healthRef}>
        ...
        <div className="news-section">
          ...
          <div className="vertical-article-list">
            {loading ? (
              <div className="loading-message">Loading health articles...</div>
            ) : error ? (
              ...
              <div className="no-articles-message">
                <p>No health articles available. Please connect to backend to load content.</p>
              </div>
            )}
          </div>
        </div>
