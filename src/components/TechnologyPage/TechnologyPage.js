...
const Home = () => {
  ...
  // Fetch technology articles on component mount
  useEffect(() => {
    const loadTechArticles = async () => {
      try {
        setLoading(true);
        setError('');
        // Fetch technology-tagged articles with full payload like Home
        const url = `http://localhost:3000/api/articles/${currentLanguage}?tag=technology`;
        ...
        if (Array.isArray(data)) {
          ...
          articles = data.technology || data.tech || data['Technology'] || Object.values(data)[0] || [];
        }
        const normalized = (articles || []).map(a => ({
          ...a,
          content: a.content || a.description || a.excerpt || a.summary || a.body || a.contentHtml || a.bodyHtml || a.html || ''
        }));
        setTechArticles(normalized);
      } catch (err) {
        setError('Failed to load technology articles. Please check your backend connection.');
        ...
        <MainContent>
          ...
          <div className="news-section">
            ...
            <div className="vertical-article-list">
              {loading ? (
                <div className="loading-message">Loading technology articles...</div>
              ) : error ? (
                    ...
                    >
                      ...
                      <div className="vertical-article-content" data-no-translate>
                        ...
                        <p className="vertical-article-description">
                          {(article.content || article.description)?.substring(0, 120)}
                          {(article.content || article.description)?.length > 120 ? '...' : ''}
                        </p>
                ...
                <div className="no-articles-message">
                  <p>No technology articles available. Please connect to backend to load content.</p>
                </div>
              )}
            </div>
          </div>
